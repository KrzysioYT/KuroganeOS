...
Path("sdk/include/kurogane/application.h").write_text(r'''#ifndef KUROGANE_SDK_APPLICATION_H
...
Path("userspace/system/app-registryd").mkdir(parents=True, exist_ok=True)
Path("userspace/system/app-registryd/main.c").write_text(r'''#include "../../runtime/user.h"
...
manifests = {
    "shell.manifest": "format=1\nid=org.kurogane.shell\nname=Terminal\nexec=/apps/shell\nflags=12\n",
    "files.manifest": "format=1\nid=org.kurogane.files\nname=Files\nexec=/apps/files\nflags=10\n",
    "monitor.manifest": "format=1\nid=org.kurogane.monitor\nname=System Monitor\nexec=/apps/monitor\nflags=10\n",
    "about.manifest": "format=1\nid=org.kurogane.about\nname=About KuroganeOS\nexec=/apps/about\nflags=10\n",
    "hello.manifest": "format=1\nid=org.kurogane.hello\nname=Hello\nexec=/apps/hello\nflags=12\n",
}
manifest_dir = Path("rootfs/apps/manifests")
manifest_dir.mkdir(parents=True, exist_ok=True)
for name, data in manifests.items():
    (manifest_dir / name).write_text(data)
...
print("application registry slice applied")
