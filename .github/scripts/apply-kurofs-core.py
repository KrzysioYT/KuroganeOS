...
Path('kernel/fs/kurofs.hpp').write_text(header)
Path('kernel/fs/kurofs.cpp').write_text(source)

todo = Path('TODO-DEFERRED-TESTS.md')
text = todo.read_text()
anchor = '## Existing deferred qualification\n'
addition = (
...
if text.count(anchor) != 1:
    raise SystemExit('TODO insertion anchor mismatch')
todo.write_text(text.replace(anchor, addition, 1))
print('KuroFS v1 metadata core applied')
