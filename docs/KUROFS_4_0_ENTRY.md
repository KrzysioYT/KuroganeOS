...
Create recovery preserves a pending child when parent publication is ambiguous. Mount turns it into `LIVE` only when the generation-bound directory record persisted, otherwise into `ORPHAN`; this prevents rollback from tombstoning a child that a durable parent already references.
