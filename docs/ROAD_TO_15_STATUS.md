...
The current engineering slice adds bounded MSI-X route groups and a separate two-vector e1000e gate. Its implementation and local evidence are tracked in [PCI_MSIX_GROUPS.md](PCI_MSIX_GROUPS.md). Group transport still requires exact-source Actions qualification; production-driver group adoption, SMP and the broader hardware matrix remain open. Steel is not yet qualified.
