...
Active work is built on persistent KuroFS allocation primitives over the production `storage::block::Device` contract. The current unqualified engineering slice includes revision-checked regular-file growth, zero-filled expansion, truncate-to-zero, contiguous in-place extension, copy-on-grow relocation and post-publication extent reclamation. Directory copy-on-grow returns its superseded extent to the allocator. Copy-on-write unlink now compacts the parent, refuses non-empty directories, retires the detached inode with an advanced generation and only then reclaims data. Remount, stale-writer, no-space, tombstone-reuse and reclaimed-range tests pass locally; formal 4.0 same-SHA qualification remains pending.
