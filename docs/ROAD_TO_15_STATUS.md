...
Bounded MSI-X route groups passed exact-source Actions run `34811940628` at `d42d9e31e98fbcb39577184475e36d97cb77d251`, together with Pre-Steel closeout `34811940844`. The contract and evidence are tracked in [PCI_MSIX_GROUPS.md](PCI_MSIX_GROUPS.md). Active work adopts groups for independent VirtIO RX/TX notifications, with shared-vector and polling fallback; that extension awaits its own runtime matrix. SMP and broader hardware qualification remain open. Steel is not yet qualified.
