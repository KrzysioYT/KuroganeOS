...
Status initialize_vfs_backend(const vfs::FileSystem& backend) {
    ...
    const vfs::Status open_status = vfs::open(
        &g_vfs, &g_path_context, kConfigurationPath, vfs::OpenFlags::Read, &handle);
    ...
    const vfs::Status read_status = vfs::read(
        &g_vfs, handle, g_configuration, sizeof(g_configuration) - 1U, &bytes_read);
...
vfs::Status getcwd(
    ...
    if (!g_mounted) return vfs::Status::NotInitialized;
    VfsGuard guard{};
    return vfs::getcwd(context, buffer, capacity, required_size);
}
...
vfs::Status stat(
    ...
    if (!g_mounted) return vfs::Status::NotInitialized;
    VfsGuard guard{};
    return vfs::stat(&g_vfs, context, path, info);
}

vfs::Status stat(const char* path, vfs::FileStat* info) {
    return stat(&g_path_context, path, info);
}
...
vfs::Status read_file(
    ...
    VfsGuard guard{};
    vfs::FileStat info{};
    vfs::Status status = vfs::stat(&g_vfs, &g_path_context, path, &info);
...
vfs::Status open(
    ...
    if (!g_mounted) return vfs::Status::NotInitialized;
    VfsGuard guard{};
    return vfs::open(&g_vfs, context, path, flags, handle);
}
...
vfs::Status open(
    ...
    vfs::OpenFlags flags,
    vfs::OpenFileHandle* handle) {
    return open(&g_path_context, path, flags, handle);
}
...
vfs::Status create(const vfs::PathContext* context, const char* path) {
    if (!g_mounted) return vfs::Status::NotInitialized;
    VfsGuard guard{};
    return vfs::create(&g_vfs, context, path);
}
vfs::Status create(const char* path) { return create(&g_path_context, path); }

vfs::Status unlink(const vfs::PathContext* context, const char* path) {
    if (!g_mounted) return vfs::Status::NotInitialized;
    VfsGuard guard{};
    return vfs::unlink(&g_vfs, context, path);
}
vfs::Status unlink(const char* path) { return unlink(&g_path_context, path); }
...
vfs::Status rename(
    ...
    if (!g_mounted) return vfs::Status::NotInitialized;
    VfsGuard guard{};
    return vfs::rename(&g_vfs, context, source_path, destination_path);
}
vfs::Status rename(const char* source_path, const char* destination_path) {
    return rename(&g_path_context, source_path, destination_path);
}

vfs::Status mkdir(const vfs::PathContext* context, const char* path) {
    if (!g_mounted) return vfs::Status::NotInitialized;
    VfsGuard guard{};
    return vfs::mkdir(&g_vfs, context, path);
}
vfs::Status mkdir(const char* path) { return mkdir(&g_path_context, path); }

vfs::Status rmdir(const vfs::PathContext* context, const char* path) {
    if (!g_mounted) return vfs::Status::NotInitialized;
    VfsGuard guard{};
    return vfs::rmdir(&g_vfs, context, path);
}
vfs::Status rmdir(const char* path) { return rmdir(&g_path_context, path); }
