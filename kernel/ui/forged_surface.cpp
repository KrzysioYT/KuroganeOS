        ...
        switch (widget.type) {
            case KU_UI_WIDGET_PANEL: {
                ...
                outline_chamfered(
                    ...
                    selected || focused ? kHotEdge : kSteelEdge,
                    fill);
                graphics::fill_rect(row.x + 8, row.y + 1,
                                    row.width > 116 ? 104 : row.width - 16,
                                    1, selected || focused ? kHotEdge : kCrimson);
                graphics::fill_rect(row.x + 5, row.y + 7, 2,
            ...
            case KU_UI_WIDGET_LABEL:
                ...
                font::draw(face, text_x, row.y + 6, widget.text,
                           disabled ? kMuted : kAsh, background, 1U, true);
                break;
            case KU_UI_WIDGET_BUTTON:
            case KU_UI_WIDGET_INPUT:
