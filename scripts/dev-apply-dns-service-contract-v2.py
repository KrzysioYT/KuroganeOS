...
static ku_status_t transact_request(
    ...
    if (request == (const ku_dns_service_request*)0 ||
        response == (ku_dns_service_response*)0) return KU_STATUS_INVALID_ARGUMENT;
...
static void initialize_request(
    ...
    if (host != (const char*)0) (void)copy_host(request->host, sizeof(request->host), host);
}
...
__attribute__((noreturn)) void _start(void) {
    ...
    for (attempts = 0U; attempts < DNS_PROBE_WAIT_ATTEMPTS; ++attempts) {
        ...
        if (status == KU_STATUS_OK && network.ready != 0U &&
            network.address[0] != 0U && nonzero_address(network.dns)) break;
        (void)ku_sleep(1U);
    }
    if (attempts == DNS_PROBE_WAIT_ATTEMPTS) fail_public(1U);

    public_request.structure_size = sizeof(public_request);
    if (!copy_host(public_request.host, sizeof(public_request.host), "example.com")) fail_public(2U);
    if (ku_dns_resolve_a(&public_request) != KU_STATUS_OK ||
        !nonzero_address(public_request.address)) fail_public(3U);

    {
        ...
        if ((ku_status_t)ku_syscall3(
                ...
                sizeof(invalid),
                0U) != KU_STATUS_VERSION_MISMATCH) fail_public(4U);
    }

    {
        ku_dns_a_request unknown = {0};
        unknown.structure_size = sizeof(unknown);
        if (!copy_host(unknown.host, sizeof(unknown.host),
                "kurogane-os-does-not-exist.invalid")) fail_public(5U);
        if (ku_dns_resolve_a(&unknown) != KU_STATUS_NOT_FOUND ||
            !zero_address(unknown.address)) fail_public(6U);
    }
    ...
    if (transact_request(connection, &request, &response) != KU_STATUS_OK ||
        ...
        !nonzero_address(response.address)) fail_service(2U);

    {
        const uint8_t malformed = UINT8_C(0xa5);
        if (ku_service_send(connection, &malformed, sizeof(malformed)) != KU_STATUS_OK) fail_service(3U);
        clear_bytes(&response, sizeof(response));
        if (receive_response(connection, &response) != KU_STATUS_OK ||
            response.status != KU_STATUS_CORRUPT_DATA || response.request_id != 0U ||
            !zero_address(response.address)) fail_service(4U);
    }

    initialize_request(&request, UINT64_C(2), UINT32_C(0xffffffff), "unsupported.invalid");
    if (transact_request(connection, &request, &response) != KU_STATUS_OK ||
        response.status != KU_STATUS_NOT_SUPPORTED || response.request_id != UINT64_C(2) ||
        !zero_address(response.address)) fail_service(5U);

    initialize_request(&request, UINT64_C(3), KU_DNS_SERVICE_RESOLVE_A, (const char*)0);
    for (attempts = 0U; attempts < sizeof(request.host); ++attempts) request.host[attempts] = 'x';
    if (transact_request(connection, &request, &response) != KU_STATUS_OK ||
        response.status != KU_STATUS_INVALID_ARGUMENT || response.request_id != UINT64_C(3) ||
        !zero_address(response.address)) fail_service(6U);

    initialize_request(&request, UINT64_C(4), KU_DNS_SERVICE_RESOLVE_A,
        "kurogane-os-does-not-exist.invalid");
    if (transact_request(connection, &request, &response) != KU_STATUS_OK ||
        response.status != KU_STATUS_NOT_FOUND || response.request_id != UINT64_C(4) ||
        !zero_address(response.address)) fail_service(7U);
    ...
    for (round = 0U; round < DNS_PROBE_CLEANUP_ROUNDS; ++round) {
        ...
        connection = (ku_service_connection_t)connected;
        initialize_request(&request, UINT64_C(100) + round, UINT32_C(0xffffffff), "cleanup.invalid");
        if (transact_request(connection, &request, &response) != KU_STATUS_OK ||
            response.status != KU_STATUS_NOT_SUPPORTED ||
            response.request_id != UINT64_C(100) + round) fail_service(10U);
    ...
    if (ku_service_close((ku_service_connection_t)connected) != KU_STATUS_OK) fail_service(13U);
...
print("DNS service contract patch v2 applied")
