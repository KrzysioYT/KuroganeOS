...
# Native UI v3 icon vocabulary for real filesystem objects.
replace_once(
...
files_source = r'''#include "../common.h"
...
'''
write("userspace/gui/files/main.c", files_source)

# Host contracts: Files must now enumerate the live VFS and render a grid.
replace_once(
...
# Ensure new v3 icons remain available to libui tile validation.
ui_header = read("sdk/include/kurogane/ui.h")
ui_cpp = read("kernel/ui/ui.cpp")
runtime = read("kernel/user/runtime_base.inc")
files = read("userspace/gui/files/main.c")
for needle in ("KU_UI_NATIVE_ICON_FOLDER = 8", "KU_UI_NATIVE_ICON_DOCUMENT = 9", "KU_UI_NATIVE_ICON_COUNT = 10"):
    if needle not in ui_header:
        raise SystemExit(f"ui.h: missing {needle}")
for needle in ("AppIcon::Folder", "AppIcon::Document"):
    if needle not in ui_cpp:
        raise SystemExit(f"ui.cpp: missing {needle}")
if "version >= KU_UI_NATIVE_VERSION_3" not in runtime or "UINT32_C(8)" not in runtime:
    raise SystemExit("runtime: v2/v3 icon compatibility gate missing")
for needle in ("ku_file_readdir", "KU_STATUS_END_OF_STREAM", "flux_files_directory_grid: PASS"):
    if needle not in files:
        raise SystemExit(f"files: missing {needle}")

print("Flux Files Explorer migration applied")
